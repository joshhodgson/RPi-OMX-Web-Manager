  ...
  if (!_supportsMultipleNativeLoop && (loop || _currentVideos.length > 1)) {
    ...
    respawn = function () {
      ...
      if (nextVideo) {
        ...
        console.log('line 188. pid:' + _omxProcess.pid)
        _omxProcess.stdout.on('data', function(data) {console.log('data')});
    ...
    _omxProcess.stdout.on('data', function(data) {
    //  console.log('Data!')
      var buffer = new Buffer(data);
			var re = /M:[\s]*(\d+)/gm;
    //  console.log(buffer.toString())
			var result = buffer.toString().match(re);
			//if (result && totalTime>0 && result.length>0)
      if (result) {
				var last = result[result.length-1];
        var time = parseFloat(last.replace(/\D/g,''))/1000000
        if (time > 2) {
          _this.emit('progress',time)
        }
    ...
    //  _this.emit('progress',last.replace(/\D/g,''))

    //  console.log(last.replace(/\D/g,''))

      }


    });
    _omxProcess.stderr.on('data', function(data){
			var buffer = new Buffer(data);
            ...
            _this.emit('totaltime', totalTime/1000000);
...
/*
_this.videoDuration = function(videos){
  var video = _resolveVideosPath([videos]);
  var that = this

  _getVideoDuration(video, function(seconds){
